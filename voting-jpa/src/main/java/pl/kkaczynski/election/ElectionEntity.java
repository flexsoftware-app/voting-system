...
public class ElectionEntity extends AuditableEntity {
    ...
    public ElectionEntity(String name, SelectionType selectionType, Instant votingStartsAt, Instant votingEndsAt) {
        this.name = name;
    ...
    void validateTimeWindow() {
        if (votingEndsAt == null || votingStartsAt == null) {
            throw new TimeWindowException("Voting window should be set");
        }
        if (votingStartsAt.isAfter(votingEndsAt)) {
            throw new TimeWindowException("Voting starts at should be before voting ends at");
        }
    }
    ...
    public List<ElectionOptionEntity> getElectionOptionsEntities() {
